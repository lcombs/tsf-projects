...
We can compute the percentage of times we miss the intervals using the past data (abs(resid)>1.96). It is close to 5% ($\approx 4.78%$ for ARIMA-ARCH, $\approx 5.70%$ for ARIMA), so our interval is approximately wide enough to accommodate our data in both cases. However, capturing the volatility has allowed us to reach the 5% threshold and improved the forecast compared to the ARIMA only model. 
